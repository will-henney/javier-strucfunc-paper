...
\begin{equation}\label{eq:ffs}
  \color{red}
  S(r) =
  \left[ \frac{1}{1 + a_1 s_0 / r_0} \right]
  \left[ \frac{1}{1 + (a_2 s_0 / r)^{a_3}} \right]
  ,
  %  S(r) = \frac{
  %   e^{-s_0 / r_0}
  % }{
...
\textcolor{red}{%
  ...
  (see Appendix~\ref{sec:degr-struct-funct}).
...
The maximum \(B(r)\) value is \SI{23}{km^{2}.s^{-2}} at \(r = \SI{0.30}{pc}\),
...
as mentioned above.
Also consistent with this is a hook-like downturn in \(B(r)\) for \(r > 2 pc\)
for the EON region.
...
The power-law index obtained by \citet{arthur2016turbulence} are \(m \sim 1.2 \pm 0.1\) for \halpha\ and [\ion{O}{3}] emission lines and for [\ion{S}{2}] and [\ion{N}{2}] they found \(m \sim 0.8 \pm 0.1\). 
...
the differences of order 30\% between their results and our own
are entirely due to the different methodologies employed. 
