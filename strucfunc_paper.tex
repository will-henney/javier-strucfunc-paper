...
In section \ref{sec:results} we show that each \hii{} region of our sample can be characterized with parameters that are particular to each velocity field.
We used these parameters to estimate the scaling relations between them and the main physical properties of each region.
To accomplish this we implement a hierarchical Bayesian model for fitting a straight line using the \texttt{linmix} Python library which is based on the procedure of \cite{2007ApJ...665.1489K}.
The physical properties in Table \ref{tab:regions-properties} are used as the independent variable \(X\) while the derived turbulent parameters of Table \ref{tab:Res} are used as the dependent variable \(Y\).
In the following analysis we left out the EON to avoid redundancy of the regions.
For physical properties values where uncertainties are not reported (size and luminosity), we consider an error of \num{10}\% of the value.
The same criteria is used for the uncertainty values of the mean non-thermal linewidths for 30 Doradus and NGC 346.
For the turbulent parameters we use the relative error considering the 2-\(\sigma\) interval. 
Our main correlation results are shown in Table \ref{tab:RestStats} in ascending order for the \(p\)-value.
