...
The blue horizontal and vertical dashed lines show the true \(r_{0}\) and the value of \(\text{B}(r) / \sigma^2 = 1 \), respectively.
...
Then we calculate the average velocity difference of the original map, \(\Delta v \), and the smoothed map,b\(\tilde{\Delta v}\), in the points \( x = \{0, \delta \} \).
This expansion gives a term that modifies the final form of the structure function, \(\text{\~B} (r)\), where the structure function without the seeing, \(\text{B}(r) \), is corrected by the term \(S_0(r; s_0)\) which has the form: 
...
The equation \ref{eq:seeingemp2} is used to correct the observational structure function, \(\text{B}(r)\). 
