...
\begin{abstract}
  Fluctuations of centroid velocities on the plane of the sky, which can be derived from spectroscopic and Fabry-Perot observations, are a powerful tool for studying the turbulent dynamics of emission line regions.
  ...
  We determine the second-order structure function of the observational centroid velocity \halpha\ two-dimensional map, and the observational structure function is fitted in a non-linear least square regression using a heuristic model for the second-order structure function as an objective function.
  Finally we establish confidence intervals for the parameters provided by the fitting.
  The model we use considers: nuisance parameters like the observational seeing (\(s_0\)) and the instrumental noise (\(B_{\text{noise}}\)), and the turbulent velocity field parameters like the velocity dispersion of the centroid velocity (\(\sigma\pos\)), the autocorrelation length (\(r_0\)) and a power-law index \(m\).
  Our results are: \num{0.2}\(< s_0 \text{(FWHM) } <\)\SI{3.0}{arcsec},  \num{0.003}\(< B_{\text{noise}} <\)\SI{6.0}{km^{-2}.s^{-2}}, \num{3} \(< \sigma\pos <\) \SI{19}{km.s^{-1}},   \num{0.09}\(< r_0 <\) \SI{12}{pc} and  \num{0.7} \(< m <\)\num{1.5}. 
  We also find the relations \(\sigma\pos \propto L(H_\alpha)^{0.28 \pm 0.1}\), and \(r_0 \propto D^{0.95 \pm 0.32}\).
  A comparison between \(\sigma\pos\) and the mean non-thermal linewidth, \(\langle \sigma_{los} \rangle\), shows that these values increase following a linear monotonically relation.
%  Since the previous scale-relations results are within the uncertainty of previous investigation we conclude that the parameters obtained from our proposed model represents true kinematic properties of each region.
 % Obtaining in a reliable method the parameters of an observational structure function in turn can be used to better characterize with certain accuracy the centroid velocities fluctuations which may reflect the turbulent nature of a \hii{} region.
    %which have a strong influence on the slope that characterize the structure function behavior.

  
  
  %our methodology guarantees consistency for comparison between the galactic and giant HII regions.
\end{abstract}
...
We use \citet{1987A&A...176..347H} data to analyze the Extended Orion Nebula.
Their observational data was obtained using a 106 cm-Cassegrain telescope at Observatorium Hoher List. 
...
\midrule
Orion     & 0.436 \pm 0.02  & 5 \pm 0.5    &    37.18       &    9.9 \pm 1.2\FNa        \\
...
with data taken from the literature. Further details on each individual source and references are given in Appendix~\ref{sec:notes-individual-hii}.
...
For most of our sources \(B\noise / \sigma^2\los < 0.03 \) and the noise
...
for which the noise is sufficiently large that the observed
structure function slope is significantly shallower than
the inferred slope \(m\) of the true model.
These sources also show the largest fluctuations of the
observed \(B(r)\) around the smooth fit
...
We present the most important correlations between the turbulent parameters and the physical properties from the \hii{} region sample.
...
In the Figures the black solid line is the input regression line which is described by the equation in the plot, and the red thin lines are a random sample (\num{ \sim 400}) from the posterior distribution within one standard deviation from the slope and the \(Y\)-intercept value.
...
\caption{The log \(\sigma\pos\) - log $L(H_{\alpha})$ plane derived form our results an data in the literature. The markers indicates galactic (triangles), Magellanic (squares) and extragalactic (circles) regions. The black solid line is the input regression line which is described by the equation in the plot, and the red thin lines are a random sample (\num{ \sim 400}) from the posterior distribution within one standard deviation from the slope and the \(Y\)-intercept value. }
\label{fig:sigvsl}
\end{figure}
...
\caption{
Same information as Fig. \ref{fig:sigvsl} but for the log \(r_0\) - log \(S\) plane derived form our results an data in the literature. 
}
\label{fig:rvsR}
\end{figure}
...
\caption{
Same information as Fig. \ref{fig:sigvsl} but for the \(\sigma_{\text{pos}}\)-\(\langle \sigma_{\text{los}} \rangle\) plane. The dashed line corresponds to the linear fit obtained by \citet{2011MNRAS.413..705L} which correspond to a sample of small-to-intermediate size \hii{} regions in M33. 
}
...
Since this relation is common in the literature it can be use to investigate how reliable are our results and can be used as comparison with the previous investigations.
The fit we obtained give us a relation of \(\log \sigma\pos = (0.27 \pm 0.10) \log \text{L}(\text{H}_{\alpha})-(9.9 \pm 4.0)\).
Since it is customary in the literature to present \(\text{L} (\text{H}_{\alpha})\) as the dependent variable we also calculate this relation.
Taking the inverse of slope we have \(\text{L}_{\text{H}_{\alpha}} \propto \sigma^{3.6 \pm 1.4}\). 
Using the same procedure in \textit{linmix} and changing the variables we obtain \(\log \text{L}(\text{H}_{\alpha}) = (2.8\pm 1.0) \log \sigma\pos -(36 \pm 4)\).
Both results are close to the \(\text{L}_{\text{H}} \propto \sigma^{4}\) theoretical relation.

The fact that this relationship holds in our results is interesting considering that the \(\sigma\pos\) is obtained using the structure function and the \(\text{L}(\text{H}_{\alpha})\) values comes from different sources unrelated to oour data (see the Appendix~\ref{sec:notes-individual-hii}).
This means that the proposed model describes within certain accuracy the velocity field fluctuations and that the parameters obtained can be interpreted a true property of each \hii{} region velocity field .
...
The dependence on \(r_0\) and the diameter can be interpreted in terms that each region presents a characteristic energy injection mechanism (or combination) and despite the presence of the different mechanisms in the regions there some unique combination that drives the input energy in each region. 
...
On the other hand, we obtain \(\log \sigma\pos = (0.25 \pm 0.17) \log D - (0.2 \pm 0.38)\) which despite the large uncertainty is comparable with previous investigations within the error.
...
The power-law index \(m\) shown in the last column of Table \ref{tab:Res} is used for comparison between the observations and the theoretical value of 0.67.
...
As for any relation of the power-law index \(m\) we can see from table \ref{tab:RestStats} that this parameter does not correlate with any other parameter or physical property, being the least reliable for determining or concluding something about the nature of the physical processes inside the regions.
