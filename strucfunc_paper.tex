...
\begin{table}
  ...
  \setlength\partabwidth{0.7\linewidth}
  \begin{tabular*}{\partabwidth}{@{\extracolsep{\fill}}l rr}
    ...
    Note: \(\max[B]\) and \(\min[B]\) are over all bins in the observed structure function.
    }
  \end{tabular*}
\end{table}
...
\citep{Doi:2004a}.
However, this can be explained by the fact that the velocity map is constructed
...
Given, this the model-derived value of \(s_0\) for this source is reasonable.
...
The black solid line shows the input regression line and the red lines show some samples from the posterior distribution.
