...
\caption{
  ...
  into pixels of size \(0.5 r_0\) before calculating the PDF to suppress
  fluctuations at scales smaller than the correlation length.
  Panel h plots the ratio of velocity dispersions (see Figure~\ref{fig:los-vs-pos})
  against the RMS fractional brightness fluctuations calculated from the PDFs.
}
\label{fig:brightness-pdfs}
\end{figure}


In order to empirically disentangle the relative importance
of these three mechanisms, we require high quality measurements
...
We therefore have an embarrassment of 
