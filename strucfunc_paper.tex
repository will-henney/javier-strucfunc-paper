...
Although this work is not conclusive about the nature of photoionized gas turbulence and a precise description of the origin and implications of these turbulent motions is out of the scope of our investigation, we believe that the tools presented here (see section~\ref{sec:methods-apply}) guarantee a concise way for a comparison of a sample of different regions.
The following sections addresses a vast series of turbulent problems that could be unravel, among other kinematical research, by expanding the sample of \hii{} regions.
...
It is possible to consider from our sample the smallest and largest region for a comparison on the impact of the previous mechanism.
The Orion core is affected by the stellar wind from \(\theta^1\) Ori C that will confine even the high ionization emission lines to a thick shell.
...
High emission knots on the region encompasses supersonic global velocity dispersion values of 14 $<$ \(\sigma_{\text{los}}\) $<$ 20 km s\(^{-1}\).
...
This shells is 40 pc in diameter with an expansion velocity above 50 km s\(^{-1}\) in which the scales is related to the presence of strong stellar winds caused by the massive stars in the region.
...
Though the correlation length been particular to each region, and been correlated mainly with the size (see section~\ref{sec:scaling-relations}), it still needs further studies to reveal the true implication of this property.
...
%Note 26/01/22: Expand and polish from here
