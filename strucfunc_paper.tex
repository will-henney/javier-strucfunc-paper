...
In the following analysis we left out the EON to avoid redundancy in the fitting.
For the physical properties we consider an uncertainty of the \num{10}\% of the value and for the turbulent parameters we use the 2-\(\sigma\) intervals. 
...
In the Figures \ref{fig:sigvsl} and \ref{fig:rvsR} we show the correlations with a significance level \(p < \num{0.05}\) which corresponds to the first two rows in Table \ref{tab:RestStats}.
In the Figures the black solid line shows the input regression line and the red thin lines show some samples from the posterior distribution.
...
Our results give us a relation of \(\log \sigma\pos = (0.26 \pm 0.11) \log \text{L}(\text{H}_{\alpha})-(9.3 \pm 4.3)\).
It is customary in the literature to present \(\text{L} (\text{H}_{\alpha})\) as the dependent variable.
In our case if we choose the upper uncertainty and take the inverse of slope we have \(\text{L}_{\text{H}_{\alpha}} \propto \sigma^{2.6}\). 
Using the same procedure in \textit{linmix} and changing the variables order we obtained \(\log \text{L}(\text{H}_{\alpha}) = (3.9\pm 2.6) \log \sigma\pos -(35.55 \pm 2)\) which is within the uncertainty of the previously proposed \(\text{L}_{\text{H}} \propto \sigma^{4}\) relations.
...
%Size vs centroid velocity dispersion
...
Table \ref{tab:RestStats} shows this result for our sample in row 3. 
...
On the other hand, we obtain \(\log \sigma\pos = (0.24 \pm 0.17) \log S - (0.24 \pm 0.38)\) which despite the large uncertainty is comparable with previous investigations.

%power-law index
Finally, as for the power-law index \(m\) we can see from table \ref{tab:RestStats} that this parameter does not correlate with any other parameter or physical property, being the least reliable parameter for determining or concluding something about the nature of the physical processes inside the regions.
...
\subsection{Relationship between plane-of-sky and mean line-of-sight velocity dispersion}\label{sec:sigmapos-vs-sigmalos}
...
\citet{arthur2016turbulence} presents the same diagram for different emission lines of the Orion core nebula showing that the value \(\sigma_{\text{los}}\) is roughly twice as the \(\sigma\pos\) (figure 14 on their work), while the \citet{2011MNRAS.413..705L} diagram, for a sample of small-to-intermediate size \hii{} regions in M33, approach unity (figure 4 on their work). 
The Figure~\ref{fig:los-vs-pos} shows that for our sample of \hii{} regions the parameters follows a \(\sigma_{\text{los}}\) = \(n \sigma_{\text{pos}}\) relation with \(3 > n > 1\).
