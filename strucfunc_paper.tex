...
\(\tilde{B}\mod(r)\) of equation~\eqref{eq:sf-functional}
...
%This \(L/2\) constrain is not used for the regions observed with MUSE.
...
% Particularly the EON observations, which are the oldest, the fitting procedure gives a noise value of \SI{5.6}{km^{2}.s^{-2}} larger that the \(\sigma^2\pos\) value of \SI{3}{km^{2}.s^{-2}}.
% The derived correlation length \(r_0 = \SI{0.05}{pc}\) is \num{6} times smaller
% than the box size, which is the minimum value for this ratio of our sample. 
% Hence we can confirm that the finite map has no effect in our results.
% The derived seeing parameter \(s_0 = \SI{0.002}{pc}\), consistent with the Orion core fit, is more than \num{250} times smaller than \(r_0\).
% This large ratio is because the separation of the pixels for the EON observations are at separations above the observational seeing, so these parameter has virtually no effect in our model for the EON case.
% The previous statement can be confirmed with the Fig. \ref{fig:corner-Galactic}b which shows the corner plot for the EON.
% The RMS seeing posterior is flat, meaning that the values in that range doesn't modify the fit.

% This behavior is also seen in the Carina nebula and Lagoon nebula.
% The observations for this regions are from the FLAMES multi-fiber instrument (section \ref{sec:flames-multi-fiber} ) in which the fibers separation  (see Figure \ref{fig:velocity-maps}) is above the observational seeing for the regions.
...
%The vertical and horizontal dotted lines shows the value of the observational considerations: the rms seeing, \(s_0\) and the noise, \(B\noise\).
...
%Figures \ref{fig:corner-example-Orion} show the corner plots \citep{2017ascl.soft02002F} of the turbulent parameters and observational constraints
%(further examples in Appendix~\ref{sec:addit-covar-corn}).
...
%The \(B\noise\) value is shown in the third column which would be the minimum squared velocity difference that could be measure by the instrument.
%The fourth column in Table \ref{tab:Res} shows the rms seeing value in parsecs, \(s_0\), which represent the minimum value of the separation which would valid for the observed structure function. 
...
\caption{The \(\sigma_{\text{pos}}\)-\(\sigma_{\text{los}}\) plane. The dotted line represents slope equal to unity and the dashed line represents a slope of two. The size of the markers represent the distance to each region.}
...
Correlations between the physical properties of \hii{} regions like size, luminosity, and the velocity dispersion have been used as tools for finding scaling relationships for these objects.
These kind of relations have also been found in dwarf galaxies and high-redshift star formation regions.
...
In recent years, \citet{2012MNRAS.422.3339W} have found that clumps and \hii{} regions follow scaling relations over the range of z = 0-2 for \halpha\ size, velocity dispersion, luminosity and mass, finding \(\sigma \propto r^{0.42}\), \(\text{L}_{\text{H}_{\alpha}} \propto r^{2.72}\) and \(\text{L}_{\text{H}_{\alpha}} \propto \sigma^{4.18}\), respectively. 
...
We implement a hierarchical Bayesian model for fitting a straight line using the \texttt{linmix} Python library based on \cite{2007ApJ...665.1489K}.
The physical properties in Table \ref{tab:regions-properties} are the independent variable \(X\) and the turbulent parameters in Table \ref{tab:Res} are the dependent variable \(Y\).
For the physical properties we consider an uncertainty of the \num{10}\% of the value and for the turbulent parameters we used 1-\(\sigma\). 
...
In the Figures \ref{fig:sigvsl} and \ref{fig:rvsR} we show the correlations with a significance level \(p < \num{0.05}\).
The black solid line shows the regression line and the red thin lines show some samples from the posterior distribution.
...
Our results give us a relation of \(\log \sigma\pos = (0.27 \pm 0.1) \log \text{L}(\text{H}_{\alpha})-(10 \pm 4.0)\).
...
Using the same procedure in \texttt{linmix} and changing the variables we obtained \(\log \text{L}(\text{H}_{\alpha}) = (3.4\pm 1.5) \log \sigma\pos -(36 \pm 1)\).
Both results are close to the previously proposed \(\text{L}_{\text{H}} \propto \sigma^{4}\) relation.
...
The relationship we found is \(\log r_0 = (0.93 \pm 0.33) \log S - (1.6 \pm 0.7)\).
...
As for the power-law index \(m\) we can see from table \ref{tab:RestStats} that this parameter does not correlate with any other parameter or physical property.
...
\caption{The log \(\sigma\pos\) - log $L(H_{\alpha})$ plane derived form our results an data in the literature. }
\label{fig:sigvsl}
\end{figure}
...
\caption{The log \(r_0\) - log \(S\) plane derived form our results an data in the literature. }
\label{fig:rvsR}
\end{figure}
