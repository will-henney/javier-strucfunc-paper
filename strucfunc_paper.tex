...
\textcolor{red}{\section{Degradation of the structure function due to observational limitations}}
...
\textcolor{red}{Then we proceed to create a 2D map from collapsing the 3D velocity field so we can take account of projection effects.
We do this by obtaining the 1st-moment map of a PPV (position-position velocity) cube created using the \texttt{make\textunderscore{}ppv} command from \texttt{turbustat}.
To create the PPV cube it was also necessary to create a 3D density field.
The same \texttt{make\textunderscore{}3dfield} command was used to create  (along with the same parameters of the velocity field) a fluctuating density field.
The density fluctuations are interpreted as emissivity fluctuations following as mentioned in section~\ref{sec:constancy-power-law}.}
...
\textcolor{red}{\subsection{Finite box effects}}
...
\textcolor{red}{\subsection{Effects of seeing}}
...
S(r; s_0, r_0) = 
  \left [
\left (1+\frac{1.25 s_0}{r_0} \right) 
\left(1+ \left (\frac{2.6 s_0}{r} \right) ^{2a} \right)
  \right ]^{-1} ,
\end{equation}
}
...
%\citep{1991IAUS..148..145W}. %Note: check the bibliographic format
