...
\end{equation}
%
where the turbulent parameters are \(r_{0}\)\ , \(\sigma^2\pos\) and \(m\), and the observational constraints \(s_0\) and \( B_{\text{noise}}\).
This procedure calculate a value \(\chi\) to be minimized scaled by the inverse of the uncertainty \(\varepsilon\) in the data.
To obtain the confidence intervals we use the minimization procedure \textit{emcee} \citep{2013PASP..125..306F} which is a \textit{python} implementation of the affine-invariant ensemble sampler for Markov chain Monte Carlo (MCMC) by \citet{2010CAMCS...5...65G}.  
%Using these procedure we obtain the posteriors for our model and the parameters in equation~\ref{eq:chi}.
...
In the plots the solid circles indicate the points we use in the non-linear regression analysis (see section~\ref{sec:methods-apply}), while the open circles are the points of the structure function that where left out.
The criteria for the previous considerations is that the structure function at large separations is affected by the inhomogeneties of the velocity field (see section-\ref{sec:limit-model-struct}), hence the reason that large separation values are not considered for the fit of our model. 
...
The vertical and horizontal dotted lines shows the value of the observational considerations: the rms seeing, \(s_0\) and the value of the noise, \(B_{\text{noise}}\).
...
To present our results in the next paragraphs the small scales would refer to the separations \(r < r_0\) and large scales would refer to separations \(r > r_0\).
...
In this case we merge the first three points.
From there until separations of \SI{0.003}{pc} all points within this range fall below the true model showing that they are affected by the seeing. 
The correlation length for the Orion core is \SI{0.07 \pm 0.01}{pc}, it has a \(\sigma^2\pos\) of \SI{12 \pm 1}{km^{2}.s^{-2}} and an index of \num{1.07 \pm 0.02}.
...
The turbulent parameters that fits our model are a \(r_0\) of \SI{1 \pm 0.2}{pc}, a \(\sigma^2\pos\) of \SI{7 \pm 1}{km^{2}.s^{-2}} and an index of \num{1.13 \pm 0.01}.
...
The correlation length is \SI{0.56 \pm 0.04}{pc} pc at a \(\sigma^2\pos\) of \SI{18 \pm 1}{km^{2}.s^{-2}}  and an index of \num{1.14 \pm 0.01}.
...
For this region the points are above the seeing value which is \SI{0.07}{pc} (\(s_0\)=\SI{0.23}{\arcsecond}) and the first ten points lie below the true model.
At separations above \SI{0.4}{pc} the observed structure functions is coincident with the true model. 
The correlation length is \SI{2 \pm 0.1}{pc} at a \(\sigma^2\pos\) of \SI{3 \pm 1}{km^{2}.s^{-2}} and an index of \num{0.76 \pm 0.02}.
...
The correlation length is \SI{4 \pm 0.2}{pc} at a \(\sigma^2\pos\) of \SI{300 \pm 9}{km^{2}.s^{-2}} and an index of \num{0.84 \pm 0.04}.
30 Doradus structure function presents a behavior of tapered fluctuations.  
...
For the observations coming from the TAURUS-II instrument there wasn't any merging of points.
The value of the seeing for this regions is \SI{0.5}{pc} (\(s_0\)=\SI{0.2}{\arcsecond}).
All the structure function points lies below the true model which would indicate that the seeing is affecting the observed structure function.
The correlation length is \SI{3.4 \pm 0.3}{pc} at a \(\sigma^2\pos\) of  \SI{10 \pm 1}{km^{2}.s^{-2}} and an index of \num{0.74 \pm 0.10}.
...
The correlation length is \SI{4 \pm 0.4}{pc} at a \(\sigma^2\pos\) of \SI{16 \pm 1}{km^{2}.s^{-2}} and an index of \num{0.88 \pm 0.14}.
The structure functions presents at large scales the behavior of periodic fluctuations with a small amplitude.
...
From this point the structure functions shows consistent values around \SI{24}{km^{2}.s^{-2}} and finally there is a rise for the last point to \SI{29}{km^{2}.s^{-2}} at a separation of \SI{91}{pc}.

In the Figure~\ref{fig:strucfunc-fit-N595}a we show the structure function of NGC 595.
The seeing is \SI{0.5}{pc} (\(s_0\)=\SI{0.12}{\arcsecond}) and for this region only the first two points are below the true model. 
The correlation length is \SI{12 \pm 1}{pc} at a \(\sigma^2\pos\) of \SI{56 \pm 2}{km^{2}.s^{-2}} and an index of \num{1.3 \pm 0.05}.
...
The seeing is \SI{1.2}{pc} (\(s_0\)=\SI{0.28}{\arcsecond}) and the structure function of this region lies below the true model for most of the separation range.
The correlation length is \SI{9 \pm 3 }{pc} at a \(\sigma^2\pos\) of \SI{68 \pm 6}{km^{2}.s^{-2}} and has an index of \num{1.3 \pm 0.05}.
