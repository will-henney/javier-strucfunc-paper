...
\begin{abstract}
  ...
  We show that for ionized regions, the latter one, can be related to density fluctuations and use its diagnostic no determine the presence of turbulence induced-density.
...
The artificial velocity maps are created using a modified version of the \texttt{make\textunderscore{}extend} command from the \texttt{turbustat} Python library \citep{Koch2019AJ....158....1K},
which creates a two-dimensional fractional Brownian Motion field
\citep{Miville-Deschenes:2003a}
based on a power-law energy spectrum in wavenumber \(k\) 
with random phases. 
...
Compared to our data (from \num{0.1} and \SI{10}{pc}), this covers the small scales between \num{0.012} and \SI{0.3}{pc}.
\citet{Chakraborty:1999a} findings are a power law behavior of \(m = 0.46\)
for all its range.
...
Our structure function falls between the two previous results: 
From a spatial scale of \SI{5}{pc} up to \SI{40}{pc} (which covers small-to-intermediate scale for previous investigations and large scales for us) and from \(B(r)\) between \num{250} to \SI{600}{km^{2}.s^{-2}}.
