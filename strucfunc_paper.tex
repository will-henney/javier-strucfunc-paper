...
with an approximate relation \(L \propto \sigma^n\) that spans
more than 5 orders of magnitude in luminosity
with a  power-law index \(n = 3\) to~\(7\)
\cite{Rozas:2006b, Moiseev:2015a}.
However, it is unclear whether a single physical mechanism
...
When the recently created star starts emitting, the energy and matter coming from it interacts with the surrounding medium, putting in motion a series of dynamical processes that have a considerable effect in the velocity field of the now HII region.
Different investigations have shown that a complex velocity structure is particularly to each region. 
The excess in the thermal broadening on the spectral lines is interpreted as disorder motions along the line-of-sight (LOS).
Large-scale motions like the expansion or rotation of the region can be accounted for, but even after these are considered still persist a random component in the region velocity field.
...
Previous investigations using the second-order structure function on the velocity field use, in majority, one HII region and their own methodology.
This has the consequences that only the same authors can compare their results in a direct way and the gap in the interpretation between authors doesn't aid the understanding of the topic.
To deal with this issue we choose 9 objects in the \halpha\ emission line and use the the same methodology in all the regions.
This give us the opportunity to compare for first time the structure function of various objects.
We use previous published observational data to characterize the velocity field o HII regions located in the Milky Way, the Magellanic clouds and in the M 33 and NGC 6822 galaxies. 
Figure \ref{fig:hii-regions} shows the images of our HII region sample and Table \ref{tab:Reg} shows the the physical properties of interest in each region. 
...
Their observations were initially published by \citet{Garcia-Diaz:2008a}. 
