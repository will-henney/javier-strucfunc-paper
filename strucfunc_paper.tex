...
\begin{table}
  \begin{center}\caption{Summary of properties of our \hii{} region sample. Diameters are taken from Table 2 of \citet{1984ApJ...287..116K}.
      \NEW{References for other properties are given in Appendix~\ref{sec:notes-individual-hii}.}
...
The 3-dimensional 2nd-order structure function slope is related to the power law index as \(m\DDD = n - 3\), but the slope, \(m\), of the 2-dimensional velocity
structure function depends not only on velocity power law index, but also on the magnitude and power law index of the emissivity fluctuations via the quantity
\(\delta\kappa\), with the relation being \(m = n - 2 + \delta\kappa\)
(see section \ref{sec:constancy-power-law}).
\citet{Brunt:2004a} found that \(\delta\kappa\) is a function of the Mach number for
hydrodynamic turbulent box simulations, but we are unaware of any analytic prescription that would allow us to calculate \(\delta\kappa\) for
the fractional Brownian motion models that we use here. 
We therefore adopt an empirical approach by varying \(n\) until the
resultant structure function slope matches a particular value in the
range \(m = 0.8 \to 1.2\)
that encompasses our observational results. 
At each step, we estimate \(m\) by fitting our idealized model structure function
...
\(r_0 = \SI{11 \pm 1.0}{pc} \)), and \(m = 1.4 \pm 0.2\).
